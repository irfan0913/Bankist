...
const displayMovements = function(movements) {
    ...
    <div class="movements__row">
      ...
      <div class="movements__value">${mov}\u{20AC}</div>
    </div>
  `;
...
const calDisplayBalance = function(acc) {
    ...
    labelBalance.textContent = `${acc.balance}\u{20AC}`;
};
// calDisplayBalance(account1.movements);
const calDisplaySummary = function(acc) {
    const income = acc.movements.filter((mov)=>mov > 0).reduce((acc, cur)=>acc + cur, 0);
    labelSumIn.textContent = `${income}\u{20AC}`;
    const outGoing = acc.movements.filter((mov)=>mov < 0).reduce((acc, cur, arr)=>acc + cur, 0);
    labelSumOut.textContent = `${Math.abs(outGoing)}\u{20AC}`;
    const intrest = acc.movements.filter((mov)=>mov > 0).map((mov)=>mov * acc.interestRate / 100).filter((int, i, arr)=>int >= 1).reduce((acc, int)=>acc + int, 0);
    labelSumInterest.textContent = `${intrest}\u{20AC}`;
};
// caldisplaySummary(account1.movements);
const createUserName = function(acc) {
    acc.forEach(function(acc) {
        acc.name = acc.owner.toLocaleLowerCase().split(' ').map((word)=>word[0]).join('');
    });
};
...
btnLogin.addEventListener('click', function(e) {
    ...
    btnTransfer.addEventListener('click', function(e) {
        ...
        if (amount > 0 && receiverAcc && currentAccount.balance >= amount && receiverAcc?.name != currentAccount.name) {
            //Treanfer
...
const { withdraw, deposit } = accounts.flatMap((acc)=>acc.movements).reduce((sum, cur)=>{
...
// });
//FlatMap()
const overBalance = accounts.flatMap((acc)=>acc.movements).reduce((acc, cur)=>acc + cur, 0);
...
//# sourceMappingURL=index.d0f7c29f.js.map
